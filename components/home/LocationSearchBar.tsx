...
import { type PlaceDetails } from "@/hooks/useGoogleGeocoder";
...
export function LocationSearchBar() {
  ...
  return (
    <div className="w-full">
      <div className="bg-background rounded-lg md:rounded-xl shadow-xl p-3 sm:p-4 md:p-5 lg:p-6 border border-border/50">
        ...
        >
          ...
          <div className={hasSubcategory ? "w-full relative" : "flex-1 relative min-w-0 md:min-w-[200px]"}>
            <MapPin className="absolute left-2.5 md:left-3 top-1/2 -translate-y-1/2 h-4 w-4 md:h-5 md:w-5 text-muted-foreground z-10" />
            {!isLoaded && isPublicLocationSearchConfigured() && location.length >= 2 && (
              <Loader2 className="absolute right-2.5 md:right-3 top-1/2 -translate-y-1/2 h-3 w-3 md:h-4 md:w-4 text-muted-foreground animate-spin" />
            )}
            <Input
              ref={inputRef}
              type="text"
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 flex-shrink-0">
              ...
              <Button
                size="sm"
                className="h-10 md:h-12 px-3 md:px-4 lg:px-6 font-medium bg-[hsl(var(--red-accent))] hover:bg-[hsl(var(--red-accent))]/90 text-[hsl(var(--red-accent-foreground))] whitespace-nowrap text-xs md:text-sm disabled:opacity-50 disabled:cursor-not-allowed"
