...
import {
  ...
  RESALE_TEAL,
  groupResaleMachinesByCategory,
  resaleMachineHref,
  type ResaleMachine,
  type ResaleMachineCategory,
} from "@/lib/machineResale/machineResaleHubCatalog";
import { fetchResaleHubData, type ResaleHubData } from "@/lib/machineResale/machineResaleHubApi";
import { getMachineRentalCategoryArt } from "@/lib/machineRental/machineRentalCategoryArt";
import machineResaleHeroImg from "@/assets/services/machine-rental.png";

const EMPTY: ResaleHubData = { categories: [], machines: [], providers: [] };

function CategoryCard({ category }: { category: ResaleMachineCategory }) {
  const art = getMachineRentalCategoryArt(category.id) || getMachineRentalCategoryArt(category.name);
  return (
    <Link
      href={`/machine-resale/${encodeURIComponent(category.id)}`}
      data-resale-category-card
      className="group flex w-[132px] shrink-0 flex-col items-center gap-2 rounded-2xl border border-teal-200/80 bg-white p-3 shadow-sm transition hover:border-teal-500/40 hover:shadow-md sm:w-[148px]"
    >
      <span className="relative flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border border-teal-100 bg-white shadow-sm transition group-hover:scale-105 sm:h-[72px] sm:w-[72px]">
        <Image src={art} alt={category.name} fill sizes="72px" className="object-contain p-2" />
      </span>
...
function MachineCard({ machine }: { machine: ResaleMachine }) {
  const href = resaleMachineHref(machine);
  return (
    <Link
      href={href}
      className="flex w-[176px] shrink-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-teal-700/30 hover:shadow-md sm:w-[200px]"
    >
      <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden bg-teal-50">
        {machine.imageUri ? (
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-3">
        <p className="truncate text-xs font-semibold leading-snug text-slate-900 sm:text-[13px]" title={machine.name}>
          {machine.name}
        </p>
        {machine.priceLabel ? (
          <p
            className="mt-auto truncate text-[11px] font-semibold leading-snug text-teal-900 sm:text-xs"
...
export function MachineResaleHub() {
  ...
  const scrollCategories = useCallback((direction: "left" | "right") => {
    ...
    if (!el) return;
    const card = el.querySelector<HTMLElement>("[data-resale-category-card]");
  ...
  useEffect(() => {
    const id = window.setInterval(() => {
      setPlaceholderIndex((i) => (i + 1) % RESALE_SEARCH_PLACEHOLDERS.length);
    }, 2600);
  ...
  const groupedMachines = useMemo(
    () => groupResaleMachinesByCategory(data.categories, data.machines),
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: RESALE_CANVAS }}>
      <section
        ...
        style={{
          background: `linear-gradient(160deg, ${RESALE_TEAL} 0%, #0d9488 55%, #115e59 100%)`,
        }}
      >
        <div className="pointer-events-none absolute inset-0" aria-hidden>
          <div className="absolute -left-20 top-0 h-64 w-64 rounded-full bg-teal-300/15 blur-[90px]" />
          <div className="absolute bottom-0 right-10 h-56 w-56 rounded-full bg-cyan-200/10 blur-[80px]" />
        </div>
          ...
          <div className="grid items-end gap-6 lg:grid-cols-[minmax(0,1.25fr)_minmax(0,0.75fr)] lg:gap-8">
              ...
              >
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    placeholder={RESALE_SEARCH_PLACEHOLDERS[placeholderIndex]}
            ...
            <div className="relative mx-auto hidden w-full max-w-[260px] justify-self-end lg:block xl:max-w-[290px]">
              <div className="relative aspect-square overflow-hidden">
                <Image
                  src={machineResaleHeroImg}
                  alt="Construction machines for sale"
                  fill
                  priority
      ...
      <div className="layout-shell space-y-10 py-8 pb-16">
        ...
        <section>
          ...
          {loading ? null : data.providers.length > 0 ? (
            ...
            <div className="relative mt-4 overflow-hidden rounded-2xl border border-teal-200/80 bg-gradient-to-br from-teal-50 via-white to-cyan-50 px-5 py-8 sm:px-10 sm:py-10">
        ...
        >
          <div>
            ...
            <p className="mt-1 max-w-xl text-sm text-teal-50/90">{t("bulkCtaBody")}</p>
          </div>
