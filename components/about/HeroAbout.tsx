...
const HeroAbout = () => {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`inline-flex items-center gap-2 px-3 sm:px-4 py-2 sm:py-2 rounded-full bg-background/70 backdrop-blur border text-xs sm:text-sm font-medium text-primary mb-4 sm:mb-6 transition-all duration-700 delay-200 ${
          isVisible ? "opacity-100 scale-100" : "opacity-0 scale-95"
        }`}>
          <Shield className="h-3.5 w-3.5 sm:h-4 sm:w-4 animate-pulse" />
          <span className="whitespace-nowrap">{t("about.eyebrow", "Trusted service discovery for India")}</span>
        </div>
        <h1 className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-foreground mb-4 sm:mb-6 max-w-4xl mx-auto leading-tight px-2 transition-all duration-1000 delay-300 ${
          ...
          {t("about.heroTitle", "Built to Make Finding Reliable Services Effortless")}
        </h1>
        <p className={`text-sm sm:text-base md:text-lg lg:text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed mb-6 sm:mb-8 px-2 transition-all duration-1000 delay-500 ${
         ...
         {t("about.heroDescription", "Imagineering India connects businesses and individuals with verified service providers across construction, machinery, contractors, logistics, real estate, and allied services — all through a location-first, transparent, and execution-ready platform.")}
        </p>
        <p className="text-muted-foreground text-sm sm:text-base md:text-lg lg:text-xl max-w-3xl mx-auto leading-relaxed mb-6 sm:mb-8 px-2 transition-all duration-1000 delay-500">{t("about.heroSubtext", "Whether it’s sourcing heavy equipment, hiring a contractor, arranging logistics, or finding project-ready infrastructure, we help you connect with the right provider, at the right location, at the right time.")}</p>
        <div className={`flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 mb-6 sm:mb-8 transition-all duration-1000 delay-700 ${
          ...
          <Link
            href="/services"
            className="inline-flex items-center justify-center px-6 sm:px-6 py-3 sm:py-3 rounded-lg bg-[hsl(var(--red-accent))] text-[hsl(var(--red-accent-foreground))] font-semibold text-sm sm:text-base shadow-md hover:shadow-xl hover:translate-y-[-2px] hover:scale-105 transition-all duration-300 w-full sm:w-auto group relative overflow-hidden"
          >
            <span className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700" />
            <span className="relative z-10">{t("about.exploreServices", "Explore services")}</span>
          </Link>
          <Link
            href="/about#why"
            className="inline-flex items-center justify-center px-6 sm:px-6 py-3 sm:py-3 rounded-lg border-2 border-border text-foreground font-semibold text-sm sm:text-base hover:bg-foreground/10 hover:border-primary/50 hover:scale-105 transition-all duration-300 w-full sm:w-auto"
          >
            {t("about.whyTrust", "Why customers trust us")}
          </Link>
        </div>
        <div className={`mt-6 sm:mt-12 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 max-w-4xl mx-auto transition-all duration-1000 delay-900 ${
          isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
        }`}>
          {[
            ...
            <div 
              key={index}
              className="flex items-center gap-2 sm:gap-3 justify-center rounded-lg sm:rounded-xl border bg-background/70 backdrop-blur px-3 sm:px-4 py-3 sm:py-3 text-xs sm:text-sm text-muted-foreground hover:bg-background/90 hover:border-primary/30 hover:scale-105 hover:shadow-lg transition-all duration-300 group"
