...
export function PaintShadeBrowseModal({
  ...
  useEffect(() => {
    ...
    const match = brands.find(
      (b) =>
        brand &&
        b.toLowerCase().includes(String(brand).toLowerCase().trim())
    );
    setBrandFilter(match || "");
  }, [open, brand, brands]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90vh] w-[min(96vw,42rem)] flex-col gap-0 overflow-hidden p-0 sm:max-w-2xl">
        <DialogHeader className="space-y-1 border-b px-4 py-3 text-left sm:px-5">
          <DialogTitle>Browse shades</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-3 border-b px-4 py-3 sm:px-5">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Search shade code or name…"
              className="h-10 pl-9"
              autoFocus
            />
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3 sm:px-5">
          {allShades.length === 0 ? (
            <p className="py-10 text-center text-sm text-muted-foreground">
              No shades match “{query}”. Try another code or name, or type it manually on the product page.
            </p>
          ) : (
