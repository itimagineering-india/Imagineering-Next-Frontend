  ...
  <div className="p-3 sm:p-4 md:p-6 lg:p-8 space-y-3 sm:space-y-4 md:space-y-6">
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 md:gap-4">
     <div className="min-w-0 flex-1">
      <h1 className="heading-md heading-xl heading-xl text-foreground leading-tight">
       My Services
      </h1>
     ...
     <Button
      ...
      size="sm"
      className="caption self-start sm:self-auto w-full sm:w-auto h-9 sm:h-10"
     >
      {isCheckingBusinessProfile ? (
       <Loader2 className="mr-2 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4 animate-spin" />
      ) : (
       <Plus className="mr-2 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4" />
      )}
    ...
    {!isLoading && (
     <Card className="overflow-hidden border-muted shadow-sm">
      <CardHeader className="border-b bg-muted/30 py-3 sm:py-4">
       <CardTitle className="subtitle text-foreground ">
        Listing overview
       </CardTitle>
      </CardHeader>
      <CardContent className="p-0">
       <div className="grid grid-cols-3 divide-x divide-border">
        <div className="py-4 text-center">
         <p className="heading-xl tabular-nums tracking-tight text-foreground">{serviceStats.total}</p>
         <p className="mt-1 caption uppercase tracking-wide text-muted-foreground ">
          Total
         </p>
        </div>
        <div className="py-4 text-center">
         <p className="heading-xl tabular-nums tracking-tight text-primary">{serviceStats.active}</p>
         <p className="mt-1 caption uppercase tracking-wide text-muted-foreground ">
          Active
         </p>
        </div>
        <div className="py-4 text-center">
         <p className="heading-xl tabular-nums tracking-tight text-muted-foreground">
       ...
       <p className="border-t border-border bg-muted/25 px-3 py-3 caption leading-relaxed text-muted-foreground sm:px-4 ">
    ...
    {isLoading ? (
     <Card>
      <CardContent className="py-6 sm:py-8 md:py-12 text-center px-3 sm:px-6">
       <p className="caption text-muted-foreground">Loading services...</p>
      </CardContent>
     </Card>
    ) : filteredServices.length === 0 ? (
     <Card>
      <CardContent className="py-6 sm:py-8 md:py-12 text-center px-3 sm:px-4">
       <p className="caption text-muted-foreground mb-3 sm:mb-4">
    ...
    {serviceToEdit && serviceToEdit.category && (
     <MultiStepServiceForm
      ...
      initialData={{
       ...
       priceMode: serviceToEdit.priceMode || "exact",
       pricingType: (serviceToEdit.priceType || "fixed") as any,
       startingPrice: (serviceToEdit.price || 0).toString(),
       priceMin: ((serviceToEdit.priceMin ?? serviceToEdit.price) || 0).toString(),
       priceMax: (serviceToEdit.priceMax || 0).toString(),
       availability: {
        days: [],
    ...
    <AlertDialog open={businessProfileGate !== null} onOpenChange={(o) => !o && setBusinessProfileGate(null)}>
     <AlertDialogContent className="mx-3 sm:mx-4">
      <AlertDialogHeader>
       <AlertDialogTitle className="listing-title">
        {businessProfileGate === "primary_category"
         ? "Primary category required"
         : "Business profile required"}
       </AlertDialogTitle>
       <AlertDialogDescription className="body">
        {businessProfileGate === "primary_category"
      ...
      <AlertDialogFooter className="flex-col sm:flex-row gap-2 sm:gap-0">
       <AlertDialogCancel className="w-full sm:w-auto body">Not now</AlertDialogCancel>
       <AlertDialogAction
        className="w-full sm:w-auto body"
        onClick={() => {
    ...
    <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
     <AlertDialogContent className="mx-3 sm:mx-4">
      <AlertDialogHeader>
       <AlertDialogTitle className="listing-title">Delete Service</AlertDialogTitle>
       <AlertDialogDescription className="body">
        Are you sure you want to delete this service? This action cannot be undone.
       </AlertDialogDescription>
      </AlertDialogHeader>
      <AlertDialogFooter className="flex-col sm:flex-row gap-2 sm:gap-0">
       <AlertDialogCancel className="w-full sm:w-auto body">Cancel</AlertDialogCancel>
       <AlertDialogAction 
        onClick={handleDelete} 
        className="bg-destructive text-destructive-foreground w-full sm:w-auto body"
       >
        Delete
       </AlertDialogAction>
      </AlertDialogFooter>
